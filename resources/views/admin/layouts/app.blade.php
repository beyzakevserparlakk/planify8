...
<head>
    ...
    <title>@yield('title', 'Yönetim Paneli') — Planify Admin</title>
...
<body class="bg-[#0f1115] text-gray-100 font-sans antialiased min-h-screen flex" x-data="{ sidebarOpen: false }">
    ...
    <aside class="fixed lg:static inset-y-0 left-0 z-50 w-72 bg-[#16181e] border-r border-gray-800/80 flex flex-col transition-transform duration-300 ease-in-out"
        ...
        <div class="h-20 flex items-center justify-between px-6 border-b border-gray-800/80">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2 text-2xl font-black tracking-tight text-white">
        ...
        <div class="flex-1 overflow-y-auto px-4 py-6 space-y-1.5">
            ...
            <a href="{{ route('admin.dashboard') }}"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-[#ff5528] text-white shadow-lg shadow-[#ff5528]/25' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
            ...
            <a href="{{ route('admin.events.index') }}"
               class="flex items-center justify-between px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all {{ request()->routeIs('admin.events.*') ? 'bg-[#ff5528] text-white shadow-lg shadow-[#ff5528]/25' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
            ...
            <a href="{{ route('admin.sliders.index') }}"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all {{ request()->routeIs('admin.sliders.*') ? 'bg-[#ff5528] text-white shadow-lg shadow-[#ff5528]/25' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.users.index') }}"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all {{ request()->routeIs('admin.users.*') ? 'bg-[#ff5528] text-white shadow-lg shadow-[#ff5528]/25' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.messages.index') }}"
               class="flex items-center justify-between px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all {{ request()->routeIs('admin.messages.*') ? 'bg-[#ff5528] text-white shadow-lg shadow-[#ff5528]/25' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.settings.index') }}"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all {{ request()->routeIs('admin.settings.*') ? 'bg-[#ff5528] text-white shadow-lg shadow-[#ff5528]/25' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <header class="h-20 bg-[#16181e] border-b border-gray-800/80 px-6 flex items-center justify-between sticky top-0 z-30">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="hidden sm:block text-sm font-bold text-gray-400">
                    @yield('page_title', 'Yönetim Paneli')
                </div>
            </div>
