...
@section('page_title', 'Genel Bakış & İstatistikler')
...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div class="bg-[#16181e] p-6 rounded-3xl border border-gray-800/80 shadow-xl relative overflow-hidden group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-black text-white mt-2">{{ $stats['total_events'] }}</h3>
                </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-800/60 flex items-center justify-between text-xs">
                <span class="text-emerald-400 font-bold">{{ $stats['approved_events'] }} Onaylı</span>
                <span class="text-amber-400 font-bold">{{ $stats['pending_events'] }} Bekleyen</span>
            </div>
        </div>
        ...
        <div class="bg-[#16181e] p-6 rounded-3xl border border-amber-500/20 shadow-xl relative overflow-hidden group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-black text-amber-300 mt-2">{{ $stats['pending_events'] }}</h3>
                </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-800/60 flex items-center justify-between text-xs">
                <a href="{{ route('admin.events.index', ['status' => 'pending']) }}" class="text-amber-400 hover:underline font-bold">
                    İncelemeye Git &rarr;
                </a>
            </div>
        </div>
        ...
        <div class="bg-[#16181e] p-6 rounded-3xl border border-gray-800/80 shadow-xl relative overflow-hidden group">
            ...
            <div class="mt-4 pt-4 border-t border-gray-800/60 flex items-center justify-between text-xs">
                <a href="{{ route('admin.users.index') }}" class="text-indigo-400 hover:underline font-bold">
                    Tüm Üyeler &rarr;
                </a>
            </div>
        </div>
        ...
        <div class="bg-[#16181e] p-6 rounded-3xl border border-gray-800/80 shadow-xl relative overflow-hidden group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-black text-white mt-2">{{ $stats['total_sliders'] }}</h3>
                </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-800/60 flex items-center justify-between text-xs">
                <a href="{{ route('admin.sliders.index') }}" class="text-purple-400 hover:underline font-bold">
    ...
    {{-- TABLES: RECENT EVENTS & USERS --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {{-- Son Eklenen Etkinlikler (2 cols) --}}
        <div class="lg:col-span-2 bg-[#16181e] rounded-3xl border border-gray-800/80 p-6 shadow-xl space-y-4">
            <div class="flex items-center justify-between pb-4 border-b border-gray-800">
                ...
                <a href="{{ route('admin.events.index') }}" class="text-xs font-bold text-[#ff5528] hover:underline">Tümünü Gör &rarr;</a>
            </div>
                            ...
                            @foreach($latestEvents as $event)
                                <tr class="hover:bg-white/5 transition">
                                    <td class="py-3 px-2 flex items-center gap-3">
                                        ...
                                        <div>
                                            <a href="{{ route('admin.events.edit', $event->id) }}" class="font-bold text-white hover:text-[#ff5528] line-clamp-1">
                                                {{ $event->title }}
                                            </a>
                                            <span class="text-[10px] text-gray-500">{{ $event->category ?: 'Genel' }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3 px-2 text-gray-300">{{ $event->city ?: '-' }}</td>
                                    <td class="py-3 px-2">
        ...
        {{-- Son Kayıt Olan Kullanıcılar (1 col) --}}
        <div class="bg-[#16181e] rounded-3xl border border-gray-800/80 p-6 shadow-xl space-y-4">
            <div class="flex items-center justify-between pb-4 border-b border-gray-800">
                ...
                <a href="{{ route('admin.users.index') }}" class="text-xs font-bold text-[#ff5528] hover:underline">Tümü &rarr;</a>
            </div>
